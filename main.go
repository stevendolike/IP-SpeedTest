...
var (
    ...
    speedTestURL = flag.String("url", "speed.bihai.cf/__down?bytes=500000000", "测速文件地址") // 测速文件地址
...
func main() {
	...
	if _, err := os.Stat("locations.json"); os.IsNotExist(err) {
		fmt.Println("本地 locations.json 不存在\n正在从 https://speed.bihai.cf/locations 下载 locations.json")
		resp, err := http.Get("https://speed.bihai.cf/locations")
		if err != nil {
